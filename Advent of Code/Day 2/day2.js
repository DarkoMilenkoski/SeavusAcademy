...
"3-4 m: fqmwcncsmp"]
validPassRepetitionCounter = 0;
validPassIndexOfCounter = 0;

function validatePassRepetition(num1, num2, characterSearch, pass){
    let counter = 0;
    for (let characters of pass){
        if (characters === characterSearch)
        counter++;
    }
    if (counter >= num1 && counter <= num2) return true;
    return false;
}
function validatePassIndexOf(num1, num2, characterSearch, pass){
    if (pass[parseInt(num1)] === characterSearch ^ pass[parseInt(num2)] === characterSearch) return true;
    return false;
}
...
function search(){
    ...
    index = elements[0];
    pass = elements[1];
    characterSearch = index.split(" ")[1];
    repeatSearch = index.split(" ")[0];
    num1 = repeatSearch.split("-")[0];
    num2 = repeatSearch.split("-")[1];
    if (validatePassRepetition(num1, num2, characterSearch, pass)) validPassRepetitionCounter++;
    if (validatePassIndexOf(num1, num2, characterSearch, pass)) validPassIndexOfCounter++;
    // console.log(`${characterSearch} ${repeatSearch}`);
    // console.log(elements[0].slice(-1));
    }
}
search();
console.log(validPassRepetitionCounter);
console.log(validPassIndexOfCounter);